...
function StripePaymentForm({ orderNumber, total, onSuccess }: {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="rounded-xl border border-[#B91C1C]/10 bg-white p-5 mb-4">
        <PaymentElement />
      </div>
      <button type="submit" disabled={processing || !stripe}
        className="btn-primary w-full rounded-full bg-[#7F1D1D] text-white py-4 font-bold text-lg hover:bg-[#991B1B] transition-all disabled:opacity-50">
  ...
  if (orderData) {
    return (
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-12 lg:py-20">
        ...
        {/* Order summary card */}
        <div className="rounded-2xl border border-[#B91C1C]/10 bg-hibiscus-bg p-6 mb-8">
          ...
          <div className="flex justify-between font-display font-bold text-2xl mt-2">
            <span>Total</span>
            <span className="text-[#B91C1C]">${formatPrice(orderData.total)}</span>
          </div>
        </div>
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-semibold transition-all ${
              paymentMethod === "card"
                ...
                : "bg-hibiscus-bg text-muted-foreground hover:bg-[#B91C1C]/10 border border-[#B91C1C]/10"
            }`}
          >
          ...
          <>
            {stripePromise && orderData.clientSecret ? (
              <Elements stripe={stripePromise} options={{
                ...
                appearance: { theme: "stripe", variables: { colorPrimary: "#7F1D1D", borderRadius: "10px" } },
              }}>
                <StripePaymentForm
              ...
              <div className="rounded-2xl border border-[#B91C1C]/10 bg-white p-8 text-center">
                <CreditCard className="h-10 w-10 text-[#B91C1C]/30 mx-auto mb-4" />
                ...
                <button onClick={() => setPaymentMethod("zelle")}
                  className="btn-primary rounded-full bg-[#7F1D1D] text-white px-6 py-3 font-semibold hover:bg-[#991B1B] transition-all">
                  Pay with Zelle
                </button>
              </div>
            )}
        ...
        {paymentMethod === "zelle" && (
          <div className="space-y-4">
            <div className="rounded-2xl border border-[#B91C1C]/10 bg-white p-6">
              ...
              <div className="bg-hibiscus-bg rounded-xl p-4 mb-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={copyZelle} className="flex items-center gap-1.5 text-sm text-[#B91C1C] font-medium hover:underline">
              ...
              <div className="bg-hibiscus-bg rounded-xl p-4 mb-4">
                ...
                <p className="font-display font-bold text-2xl text-[#B91C1C]">${formatPrice(orderData.total)}</p>
              </div>
              ...
              <div className="border-t border-[#B91C1C]/10 pt-4">
                <h4 className="font-semibold text-foreground text-sm mb-3">Steps:</h4>
                <ol className="space-y-2 text-sm text-muted-foreground">
                  <li className="flex gap-3">
                    <span className="w-6 h-6 rounded-full bg-[#B91C1C] text-white text-xs font-bold flex items-center justify-center flex-shrink-0">1</span>
                    <span>Open your Zelle or Venmo app</span>
                  </li>
                  <li className="flex gap-3">
                    <span className="w-6 h-6 rounded-full bg-[#B91C1C] text-white text-xs font-bold flex items-center justify-center flex-shrink-0">2</span>
                  ...
                  <li className="flex gap-3">
                    <span className="w-6 h-6 rounded-full bg-[#B91C1C] text-white text-xs font-bold flex items-center justify-center flex-shrink-0">3</span>
                    <span>Include <strong className="text-foreground">KEMZOBO — {orderData.orderNumber}</strong> in the memo</span>
                  </li>
                  <li className="flex gap-3">
                    <span className="w-6 h-6 rounded-full bg-[#B91C1C] text-white text-xs font-bold flex items-center justify-center flex-shrink-0">4</span>
            ...
            <button
              ...
              className="btn-primary w-full rounded-full bg-[#7F1D1D] text-white py-4 font-bold text-lg hover:bg-[#991B1B] transition-all flex items-center justify-center gap-2"
            >
      ...
      <form onSubmit={handleCreateOrder}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
                className="w-full rounded-lg border border-[#B91C1C]/15 bg-hibiscus-bg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#B91C1C] focus:bg-white transition-colors" />
            </div>
            <div>
          ...
          <div>
            ...
            <div className="rounded-2xl border border-[#B91C1C]/10 bg-hibiscus-bg p-5 space-y-3">
              {items.map((item) => (
              ...
              <div className="border-t border-[#B91C1C]/10 pt-3 space-y-2">
                ...
                <div className="border-t border-[#B91C1C]/10 pt-2 flex justify-between font-display font-bold text-xl">
                  <span>Total</span>
                  <span className="text-[#B91C1C]">${formatPrice(total)}</span>
                </div>
              </div>
            </div>

            {/* Payment method preview */}
            <div className="mt-6 rounded-xl bg-white border border-[#B91C1C]/10 p-4">
              <p className="text-sm text-muted-foreground mb-2">Payment methods available:</p>
              <div className="flex gap-3">
                <span className="text-xs font-medium bg-hibiscus-bg text-[#B91C1C] rounded-full px-3 py-1">💳 Card (Stripe)</span>
                <span className="text-xs font-medium bg-hibiscus-bg text-[#B91C1C] rounded-full px-3 py-1">💸 Zelle / Venmo</span>
              </div>
            </div>

            <button type="submit" disabled={createOrderMutation.isPending}
              className="btn-primary mt-6 w-full rounded-full bg-[#7F1D1D] text-white py-4 font-bold text-lg hover:bg-[#991B1B] transition-all disabled:opacity-50 flex items-center justify-center gap-2">
